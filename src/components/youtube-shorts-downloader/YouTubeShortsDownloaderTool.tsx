...
function isProbablyYouTubeShortsOrVideoUrl(input: string) {
  try {
...
export function YouTubeShortsDownloaderTool() {
  ...
  const canSubmit = useMemo(
    () => url.trim().length > 0 && isProbablyYouTubeShortsOrVideoUrl(url.trim()) && status !== 'loading',
    [status, url],
  );
  ...
  const resolve = useCallback(async () => {
    ...
    if (!isProbablyYouTubeShortsOrVideoUrl(input)) {
      addToast({
        title: 'Please paste a valid YouTube Shorts or YouTube watch link.',
        color: 'danger',
        variant: 'flat',
      });
      return;
    ...
    try {
      ...
      if (!data.ok) {
        addToast({
          title: data.message || 'Could not resolve this YouTube link.',
          color: 'danger',
          variant: 'flat',
        });
        setStatus('idle');
      ...
      console.error('YouTube resolve request failed:', e);
  ...
  const download = useCallback(async () => {
    ...
    if (!result) return;
    const name = safeFileName(result.title || 'youtube_shorts');
    setStatus('downloading');
    try {
      const res = await fetch('/api/youtube/download', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ videoId: result.videoId, name }),
      });
      ...
      console.error('YouTube download request failed:', e);
      ...
      <CardBody className="p-6">
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-2">
            <Icon icon="youtube" size={18} className="text-foreground-500" />
            <h3 className="font-sans text-base font-semibold">YouTube Shorts downloader</h3>
          </div>

          <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
            <div className="flex-1">
              <Input
                label="YouTube Shorts or video link"
                value={url}
            ...
            <div className="grid gap-4 md:grid-cols-[180px_1fr]">
              ...
              <div className="flex flex-col gap-2">
                <div className="text-sm text-foreground-700">
                  <div className="font-semibold">{result.title || 'YouTube video'}</div>
                  {result.author ? <div className="text-foreground-500">{result.author}</div> : null}
                  <div className="text-foreground-500">Source: {result.source}</div>
                </div>
