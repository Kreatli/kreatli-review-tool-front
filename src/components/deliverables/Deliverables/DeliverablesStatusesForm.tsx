...
import { DeliverablesStatusesField } from './DeliverablesStatusesField';

interface Props {
  statuses: ProjectDto['deliverableStatuses'];
  onSubmit: (statuses: ProjectDto['deliverableStatuses']) => void;
}

export const DeliverablesStatusesForm = ({ statuses, onSubmit }: Props) => {
  ...
  return (
    <FormProvider {...methods}>
      <form id="deliverables-statuses-form" noValidate onSubmit={handleSubmit((data) => onSubmit(data.statuses))}>
        <div ref={parentRef} className="flex min-h-60 w-full flex-col gap-2">
          ...
          >
            {fields.map(({ value }, index) => (
              <DeliverablesStatusesField key={value} value={value} index={index} onRemove={() => handleRemove(value)} />
            ))}
          </DragDropProvider>
